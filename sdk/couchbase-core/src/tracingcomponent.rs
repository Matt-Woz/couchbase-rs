...
use tracing::{debug_span, instrument, span, trace_span, Level, Span};
use url::Url;
...
impl TracingComponent {
    pub fn new(config: TracingComponentConfig) -> Self {
        Self {
            config: Mutex::new(config),
        }
    }

    pub fn reconfigure(&self, state: TracingComponentConfig) {
        let mut state_guard = self.config.lock().unwrap();
        *state_guard = state;
    }

    pub fn get_cluster_labels(&self) -> Option<ClusterLabels> {
        let config = self.config.lock().unwrap();

        config.cluster_labels.clone()
    }

    pub fn create_dispatch_span(&self, fields: &BeginDispatchFields) -> Span {
        let span = trace_span!(
            "dispatch_to_server",
            "db.system" = "couchbase",
    ...
    pub fn record_cluster_labels(&self, span: &Span) {
...
#[derive(Debug)]
pub struct BeginDispatchFields {
